...
export interface PortfolioProps {
  /** Extra classes for the root element. It fills its parent's width and the viewport height by default. */
  className?: string;
}
...
const Home = ({ className = '' }: PortfolioProps) => {
  ...
  const go = useCallback(
    (target: number | ((c: number) => number)) =>
      setCurrent((c) => Math.max(0, Math.min(count - 1, typeof target === 'function' ? target(c) : target))),
    [count],
  );
  ...
  const onPointerMove = (e: ReactPointerEvent<HTMLElement>) => {
    ...
    if (!d.mode) {
      ...
      if ((mode === 'next' && current >= count - 1) || (mode === 'prev' && current <= 0)) { dragStart.current = null; return; }
      d.mode = mode;
  ...
  const pageStyle = (i: number): CSSProperties => {
    ...
    const dragging = !!drag && ((drag.dir === 'next' && i === current) || (drag.dir === 'prev' && i === current - 1));
      ...
      >
        ...
        >
          ...
          {views.map((View, i) => {
            ...
            // `inert` is not in React 18's DOM typings, so it is spread in as a plain attribute.
          ...
          <div className="flex items-center gap-1">
            {pageNames.map((name, i) => (
              <button
                ...
                onClick={() => go(i)}
                className={`cursor-pointer rounded-md border-0 bg-transparent px-[.3rem] py-3 text-[.85rem] sm:px-2 sm:py-2 ${i === current ? 'font-semibold text-[color:var(--ink)]' : 'text-[color:var(--mute)]'
                  }`}
              >
                <span className="hidden sm:inline">{name}</span>
                <span
                  aria-hidden="true"
                  className={`block h-2 rounded-full sm:hidden ${i === current ? 'w-[22px] bg-[color:var(--ink)]' : 'w-2 bg-[color:var(--line)]'}`}
                />
              </button>
            ))}
          </div>
