...
const Cover = () => {
    ...
    return (
        <div className={cls.inner}>
            <p {...rise(0)} className={`${rise(0).className} m-0 mb-6 ${cls.small}`}>
                Portfolio · Makati City, Philippines
            </p>
            ...
            <div className="flex flex-col-reverse gap-6 sm:flex-row sm:items-center sm:justify-between">
                ...
                <div {...rise(180)} className={`${rise(180).className} shrink-0`}>
                    <img
                        src="/me.png"
                        alt="Kurt Lupin Orioque"
                        className="h-28 w-28 rounded-full border-2 border-[color:var(--line)] object-cover shadow-sm sm:h-36 sm:w-36 lg:h-44 lg:w-44"
                    />
                </div>
            </div>
            ...
            <div {...rise(360)} className={`${rise(360).className} mt-[clamp(1.5rem,4vw,3rem)]`}>
                ...
                <p className={`mt-6 ${cls.small}`}>
                    Drag the page, peel the corner, swipe, or use the arrow keys.
                </p>
            </div>
        </div>
    );
